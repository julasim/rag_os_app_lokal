...
| **M3 — Store-Adapter** | ✅ **Kern fertig & verifiziert** — neu `pipelines/store.py` (LanceDB: write/hybrid/`norm_id`-WHERE/delete/scan) + `pipelines/doc.py` (`RetrievedDoc`, ersetzt haystack.Document); `factory.py` neu (fastembed/ONNX-Embeddings + Shims, **kein haystack/qdrant/ollama**); `query.py` + `reranker.py` umgezogen; `__init__.py` entkoppelt. **`run_retrieve` E2E grün** (Hybrid + Filter). Query-Log-Bug (UUID→str) gefixt. **M3-Rest offen:** Write-Pfad (`ingest/pipeline._embed_and_store`), `vector_ops` (delete), API-Call-Sites (`documents`/`suggest`/`system`), `graph/l2`, `backup` — nutzen noch die (werfenden) Shims. |
