...
| **M8 — Shell/MCP/Frontend/Packaging** | 🟡 **MCP-Oberfläche fertig & verifiziert; Rest offen** — `mcp_server/server.py`: **Bearer-only** (OAuth-Branch/`_OAuthPrincipal`/`_resolve_oauth_principal` raus), **read-only** (`rag_upload` + TOTP-Gate `_require_mcp_admin_totp` entfernt), neu **`rag_overview`** (kompakte ACL-scoped Bestands-Karte) + **`norm_lookup`** (kanonische `norm_id`, trennt Geschwister-Normen), `rag_get_document` um **Volltext** (Child-Chunks reassembliert, verbatim) erweitert. **Verifiziert** (9/9: 6 read-only-Tools gelistet, `rag_upload`/OAuth/TOTP-Symbole weg, `build_mcp_app` importiert sauber), `ruff` grün. **(a) Tote Datei-Löschung fertig & verifiziert:** `mcp_server/oauth.py`+`oauth_routes.py`, `auth/totp.py`, `worker.py` **gelöscht**; alle Referenzen entfernt (main.py Mount/Branch, `mcp_server/__init__`, `api/auth_router.py` TOTP-Endpunkte, `api/users.py` `totp_enabled`, `db/models.py` OAuth-Tabellen + TOTP-Spalten, `config.py` oauth-Settings + `mcp_admin_email`). Smoke-Test grün (keine OAuth-Tabellen/TOTP-Spalten, alle Router importieren, `init_db`/`create_all` baut ohne oauth/totp, MCP 6 Tools), `ruff` grün. **(b) Frontend-Login-Bypass fertig & verifiziert:** lokale Auto-Admin-Session (`config.local_ui_autologin` + `auth/dependencies.py`), `useAuth` Auto-Login + `AppShell` `ready`-Gate, TOTP-Reste getrimmt; `npm run build` grün. **(c) pywebview-Shell CODE fertig & headless-verifiziert:** neu `app/desktop.py` (uvicorn-Thread @127.0.0.1 + WebView2-Fenster + Readiness-Poll + Vault-Picker + **Tray + Autostart(Registry) + Toast + Drag&Drop-JS-Bridge** + Minimize-to-Tray + sauberes Beenden; GUI-Libs lazy → `py_compile`/Import/Helfer grün, Reader-Guard grün). **(d) Packaging CODE fertig:** `build/` mit `make-icon.py` (ragos.ico erzeugt), `fetch-models.py` (bge-m3+Reranker Hybrid), `ragos-writer.spec`/`ragos-reader.spec` (Reader via `excludes` schlank), `installer-writer.iss`/`installer-reader.iss` (per-User, Modelle→LOCALAPPDATA, WebView2-Bootstrapper, Autostart-Task, `--role reader`), `build.ps1` (Frontend→Icon→Modelle→PyInstaller→Inno). **Offen — nur auf deinem Windows-Rechner (headless nicht baubar):** `build.ps1` laufen lassen → beide `.exe` bauen → Installer auf sauberem Profil abnehmen (GUI/WebView2/Docling-First-Run). |
