...
| **M4** | ✅ **fertig** — Embeddings über fastembed/ONNX (`factory.embed_query/embed_texts`). **Prod-Modell `intfloat/multilingual-e5-large`** (1024-dim, mehrsprachig) + e5-Query/Passage-Präfixe — beim echten Build entdeckt, dass fastembed **bge-m3 nicht** unterstützt (Test nutzt `bge-small`). |
...
| **M8e — Reader/Writer-Rollen-Split** | ✅ **fertig & verifiziert** — P0 (pyproject dreigeteilt base/writer/dev, ui_static-Build-Skript, File-Logging, Config `RAG_VAULT_PATH`/`RAG_SERVICE_ROLE`-Aliase + `app-settings.json` via `appsettings.py`). Ingest-Importe lazy (`watcher.py`, `main.py`-Lifespan, `api/suggest.py`); `store.py` Reader liest lokalen Cache an `current`-Tag, Writer den Vault; Lifespan startet Writer-Tasks (Queue/Watcher/Maintenance/Backup) bzw. Reader-Cache-Refresh (`publish.refresh_reader_cache`). **Verifiziert:** Reader importiert `main` OHNE docling/torch/fitz/magic (Import-Graph-Leck-Test); Writer publisht → Reader synchronisiert Cache → liest an `current` (3/3); `ruff` grün. |
| **M8 — Shell/MCP/Frontend/Packaging** | ✅ **KOMPLETT — beide Installer gebaut & boot-verifiziert** — `mcp_server/server.py`: **Bearer-only** (OAuth-Branch/`_OAuthPrincipal`/`_resolve_oauth_principal` raus), **read-only** (`rag_upload` + TOTP-Gate `_require_mcp_admin_totp` entfernt), neu **`rag_overview`** (kompakte ACL-scoped Bestands-Karte) + **`norm_lookup`** (kanonische `norm_id`, trennt Geschwister-Normen), `rag_get_document` um **Volltext** (Child-Chunks reassembliert, verbatim) erweitert. **Verifiziert** (9/9: 6 read-only-Tools gelistet, `rag_upload`/OAuth/TOTP-Symbole weg, `build_mcp_app` importiert sauber), `ruff` grün. **(a) Tote Datei-Löschung fertig & verifiziert:** `mcp_server/oauth.py`+`oauth_routes.py`, `auth/totp.py`, `worker.py` **gelöscht**; alle Referenzen entfernt (main.py Mount/Branch, `mcp_server/__init__`, `api/auth_router.py` TOTP-Endpunkte, `api/users.py` `totp_enabled`, `db/models.py` OAuth-Tabellen + TOTP-Spalten, `config.py` oauth-Settings + `mcp_admin_email`). Smoke-Test grün (keine OAuth-Tabellen/TOTP-Spalten, alle Router importieren, `init_db`/`create_all` baut ohne oauth/totp, MCP 6 Tools), `ruff` grün. **(b) Frontend-Login-Bypass fertig & verifiziert:** lokale Auto-Admin-Session (`config.local_ui_autologin` + `auth/dependencies.py`), `useAuth` Auto-Login + `AppShell` `ready`-Gate, TOTP-Reste getrimmt; `npm run build` grün. **(c) pywebview-Shell CODE fertig & headless-verifiziert:** neu `app/desktop.py` (uvicorn-Thread @127.0.0.1 + WebView2-Fenster + Readiness-Poll + Vault-Picker + **Tray + Autostart(Registry) + Toast + Drag&Drop-JS-Bridge** + Minimize-to-Tray + sauberes Beenden; GUI-Libs lazy → `py_compile`/Import/Helfer grün, Reader-Guard grün). **(d) Packaging fertig & GEBAUT:** `build/` (`make-icon.py`, `fetch-models.py` [e5-large+Reranker], `ragos-writer.spec`/`ragos-reader.spec`, `installer-writer.iss`/`installer-reader.iss`, `build.ps1`). **Echter Windows-Build durchgeführt:** beide Installer erzeugt (`dist/RAG-OS-Schreiber-Setup.exe` ~3 GB, `dist/RAG-OS-Leser-Setup.exe` ~2,5 GB, Inno „Successful compile") und **beide Payloads booten live** (Health 200, sqlite+lancedb; Writer startet Queue/Maintenance/Backup; Reader liest Cache). **Beim Build gefixte reale Bugs:** e5-large statt bge-m3, `aiosqlite`/uvicorn/SQLAlchemy-`collect_all` (sonst Import-Crash der eingefrorenen App), torch-Lizenzbäume-Purge (`Remove-DistInfoLicenses` in `build.ps1`), ASCII-Prints. **Rest (nur interaktiv):** WebView2-Fenster-Rendering im echten Fenster. **M8f (2026-07-21):** Docling-First-Run-Download war fragil (erster Ingest rannte mit dem HF-Download um die Wette → „Missing safe tensors file") → **Docling Layout/TableFormer + e5-Tokenizer jetzt gebündelt** (`fetch-models.py`, Installer → `models/{docling,e5-tokenizer}`, `artifacts_path` + `HF_HUB_OFFLINE` am Prozessstart). Kein Runtime-Download, air-gapped; Reader excludet sie. **E2E verifiziert (isoliert, offline):** MRG (307 Chunks) + EStG 1988 (1339 Chunks, Tabellen) = 1646 Chunks in LanceDB, kein safetensors-Fehler; Retrieval offline korrekt + ordner-scharf (Mietzins→MRG §16, Steuer→EStG Werbungskosten §16). Beide Installer neu gebaut (Schreiber 3,5 GB nach RapidOcr-Trim, Leser 2,6 GB), `ba32b28`. **M8g (2026-07-21):** Ingest-Engpass ist das **Embedding** (~1 s/Chunk, e5-large fp32 CPU = 95 %; nicht der Parser, GPU scheitert auf Py3.14). Fix: e5-large **INT8-ONNX** direkt über onnxruntime (`factory.py`, kein fastembed) → ~3,2× schneller, Modell 4× kleiner (561 MB), Qualität intakt (Vektor-Treue 0,99); fastembed aus beiden Installern → **~1,6 GB kleiner**. Dazu **Tag-Fix** (ganzer Text + Boilerplate-/Römisch-Filter statt 4000-Zeichen-Kopf). E2E: EStG-Ingest 509 s → 217 s. |
...
```
   ...
   │ • ONNX e5-INT8 (Embeddings)       │          │ • ONNX e5-INT8 + Reranker     │
...
- **Retrieval**: Hybrid (dense e5-large-INT8 + FTS/BM25 + RRF) + exaktes `norm_id`-WHERE + Norm-Referenz-Fastpath + Reranker.
